...
class TestScoreRepository:
    ...
    def test_upsert_replaces_existing(self, score_repo, sample_project):
        week_key = to_week_key(date.today())
        ps = ProjectScore(
            project_id=sample_project.id, week_key=week_key, score=50, status="red"
        )
        score_repo.upsert(ps)
        ps2 = ProjectScore(
            project_id=sample_project.id, week_key=week_key, score=90, status="green"
        )
...
class TestScoringService:
    def test_compute_and_save_returns_score(
        self, scoring_svc, sample_project, sample_sessions
    ):
